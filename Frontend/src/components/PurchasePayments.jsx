...
const PurchasePayments = () => {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      const res = await fetchPayments("purchase");
...
>
        ...
        >
          <div className="flex items-center gap-2 mb-1">
            <div className="bg-[#f5c16c]/20 p-1.5 rounded-lg">
              <FaBox className="text-[#c98c3a] text-sm" />
            </div>
            <span className="text-xs text-gray-500 font-medium uppercase tracking-wider">
              Total Qty Bought
            </span>
      ...
      <div className="bg-white rounded-2xl p-4 shadow-md border border-[#7f2c2c]/10 mb-6 flex flex-col sm:flex-row gap-4 items-stretch sm:items-center">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="Search by item, seller, supplier, invoice..."
            value={searchTerm}
        ...
        {filteredPayments.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-[#4b2e2e] text-lg font-medium">
              {payments.length === 0 ? "No purchase payments found" : "No matching records"}
            </p>
          </div>
        ) : (
              ...
              <tbody>
                {filteredPayments.map((p, i) => (
                  ...
                  >
                    ...
                    <td className="p-3">{p.purchasedBy?.name || "Sole"}</td>
                    <td className="p-3">{p.soldBy?.name || p.supplierName || "-"}</td>
                    <td className="p-3">
